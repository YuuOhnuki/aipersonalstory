...
export const api = {
  ...
  async sendMessage(sessionId: string, content: string): Promise<{ messages: Message[]; done?: boolean }> {
    try {
      return await http(`/api/chat?sessionId=${encodeURIComponent(sessionId)}`, {
        method: "POST",
      ...
      return {
        messages: [
          ...
          { id: crypto.randomUUID(), role: "assistant", content: "了解しました。続けて教えてください。", createdAt: now },
        ],
      };
    }
  },
  async getResult(sessionId: string): Promise<MBTIResult> {
    try {
      return await http<MBTIResult>(`/api/result?sessionId=${encodeURIComponent(sessionId)}`);
    } catch {
      return {
