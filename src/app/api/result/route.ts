...
export async function GET(req: NextRequest) {
    ...
    try {
        const out = await generateText(mbtiPrompt, {
            max_new_tokens: 120,
            temperature: 0.2,
        });
        axes = parseAxes(out || '');
    ...
    try {
        ...
        const fo = await generateText(fp, {
            max_new_tokens: 220,
            temperature: 0.4,
        });
        features = (fo || '').trim();
    ...
    let story = '';
    try {
        const out2 = await generateText(storyPrompt, {
            max_new_tokens: 1100,
            temperature: 0.7,
        });
        story = (out2 || '').trim();
    ...
    if (!story || story.includes(genericFallback)) {
        ...
        story =
            typeToStory[t] ||
            `ある日、あなたは自分の選択がどこから生まれるのかを見つめ直す。${t} の気質が導くのは、焦らず、しかし確かに進む歩み。小さな決断の積み重ねが、あなたの物語を静かに前へ運んでいく。`;
    }
