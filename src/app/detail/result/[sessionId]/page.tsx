...
export default async function DetailResultPage({
    ...
    const avatarUrl =
        row.avatar_url ||
        `/api/image/avatar?type=${encodeURIComponent(row.mbti_type || 'MB')}&title=${encodeURIComponent('詳細診断')}`;
    const sceneUrl =
        row.scene_url ||
        `/api/image/scene?type=${encodeURIComponent(row.mbti_type || 'MB')}&title=${encodeURIComponent('詳細診断')}`;
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="rounded-xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-md p-4 text-sm">
                                        <div className="grid grid-cols-2 gap-2 text-black/70 dark:text-white/70">
                                            ...
                                            <div>
                                                ValueFlexibility:{' '}
                                                {row.valueFlexibility}
                                            </div>
                                        </div>
                                        <div className="mt-3 text-sm text-black/70 dark:text-white/70 whitespace-pre-wrap">
                                            {row.summaryText}
                                        </div>
                                    </div>
                                </div>
