...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    api.startSession().then((s) => {
      setSessionId(s.id);
      try { localStorage.setItem("sessionId", s.id); } catch {}
    });
  }, []);
  ...
  const onSend = async (text: string) => {
    ...
    const optimistic: Message = { id: crypto.randomUUID(), role: "user", content: text, createdAt: now };
          ...
          <Card>
            <CardBody>
              ...
              <div className="flex flex-col gap-4 max-h-[60vh] overflow-auto pr-1 transition-opacity">
                {messages.length === 0 && (
                  <div className="text-sm text-black/60 dark:text-white/60">こんにちは。まずは最近の過ごし方について教えてください（例：休日の過ごし方）。</div>
                )}
                {messages.map((m) => (
                ...
                {loading && (
                  <div className="text-xs text-black/60 dark:text-white/60">AIが考えています...</div>
                )}
                <div ref={bottomRef} />
              </div>
              <div className="mt-4">
                <MessageInput onSend={onSend} disabled={!sessionId || loading} />
              </div>
            </CardBody>
          </Card>
        </div>
