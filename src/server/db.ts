...
async function getPool() {
  ...
  try {
    ...
    if (!connectionString) {
      throw new Error("DATABASE_URL is not set. Please set your Railway Postgres URL in .env.local as DATABASE_URL.");
    }
    // Parse the URL ourselves to avoid framework URL polyfill edge cases
    let host = ""; let port = 5432; let user = ""; let password = ""; let database = "";
    try {
    ...
    console.info("[db] Connected to Postgres at", host + ":" + port, "db=", database);
    // Init schema
    await pool.query(`
...
export async function dbSaveMbtiResult(sessionId: string, axes: MBTIMap, type: string, title: string, summary: string, story: string, extras?: { features?: string; reasons?: string; advice?: string; avatar_url?: string; scene_url?: string }) {
  const p = await getPool();
  if (!p) return;
  await p.query(
    ...
    [sessionId, axes["E/I"], axes["S/N"], axes["T/F"], axes["J/P"], type, title, summary, story, extras?.features ?? null, extras?.reasons ?? null, extras?.advice ?? null, extras?.avatar_url ?? null, extras?.scene_url ?? null, new Date().toISOString()]
  );
}

export async function dbSaveDetailResult(sessionId: string, payload: {
  mbti_type: string,
...
export async function dbListMbtiResults(limit = 20) {
  const p = await getPool();
  if (!p) return [] as any[];
  const { rows } = await p.query(`SELECT * FROM mbti_result ORDER BY created_at DESC LIMIT $1`, [limit]);
  return rows;
}
...
export async function dbListDetailResults(limit = 20) {
  const p = await getPool();
  if (!p) return [] as any[];
  const { rows } = await p.query(`SELECT * FROM detail_result ORDER BY created_at DESC LIMIT $1`, [limit]);
  return rows;
}
