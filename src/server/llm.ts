...
async function generateWithGemini(prompt: string, options?: { max_new_tokens?: number; temperature?: number }) {
...
async function generateWithGroq(prompt: string, options?: { max_new_tokens?: number; temperature?: number }) {
  ...
  const body = {
    ...
    messages: [
      { role: "system", content: "You are a helpful assistant that responds in Japanese when the user writes Japanese." },
...
async function getLocalGenerator() {
  ...
  try {
    ...
    generationPipeline = await pipeline("text-generation", modelId, { dtype: "q8" });
...
export async function generateText(prompt: string, options?: { max_new_tokens?: number; temperature?: number }) {
  ...
  if (pipe) {
    ...
    const text: string = Array.isArray(out) ? out[0]?.generated_text ?? "" : out?.generated_text ?? String(out ?? "");
    return text;
  }
